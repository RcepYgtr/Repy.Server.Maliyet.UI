...
import { KapiBomService } from '../../../../../../libs/core/api/kapi/kapi-bom.service';
type TabType = 'materials' | 'rules';
@Component({
  selector: 'app-kapi-bom-detail',
  standalone: false,
  templateUrl: './kapi-bom-detail.component.html',
  styleUrl: './kapi-bom-detail.component.scss',
})
export class KapiBomDetailComponent implements OnInit {
  form!: FormGroup;
  ...
  ruleColumnDefs = [
    ...
    { width: 80, field: 'panelSayisi', headerName: 'Panel Sayısı', editable: true },
    { width: 80, field: 'yon', headerName: 'Yön', editable: true },
    { width: 100, field: 'kaplama', headerName: 'Kaplama', editable: true },
    { width: 100, field: 'kapiTipi', headerName: 'Kapı Tipi', editable: true },
    { width: 100, field: 'fotoselVarmi', headerName: 'Fotosel var mı?', editable: true },
  ...
  constructor(
    ...
    private kapiBomService: KapiBomService,
  ...
  private loadFromRoute() {
    ...
    setTimeout(() => {
      this.form.patchValue({ kapiTypeId: kapiTypeId });
    });

    this.loadBom(kapiTypeId);
  }

  async loadBom(id: number) {

    const res: any = await this.kapiBomService.getByModel(id);
  ...
  async Kaydet() {
    ...
    const payload = {
      kapiTypeId: this.form.value.kapiTypeId,
      ...
      rules: this.capacityRules.map(r => ({
        ...
        // 🔥 KOŞULLAR
        panelSayisi: r.panelSayisi == '' ? null : r.panelSayisi,
        yon: r.yon == '' ? null : r.yon,
        kaplama: r.kaplama == '' ? null : r.kaplama,
        kapiTipi: r.kapiTipi == '' ? null : r.kapiTipi,
        fotoselVarmi: r.fotoselVarmi,
    ...
    try {
      console.log(payload); 
      await this.kapiBomService.update(payload);
