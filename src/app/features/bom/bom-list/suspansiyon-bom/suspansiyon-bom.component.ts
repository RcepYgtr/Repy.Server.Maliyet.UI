...
import { SUSPANSİYON_BOM_LIST_COLUMNS } from '../../_factory/suspansiyon/suspansiyon-bom-list.column';
...
@Component({
  selector: 'app-suspansiyon-bom',
  standalone: false,
  templateUrl: './suspansiyon-bom.component.html',
  styleUrl: './suspansiyon-bom.component.scss',
})
export class SuspansiyonBomComponent implements OnInit {
  isLoading: any;
  ...
  constructor(
    ...
    private suspansiyonTypeService: SuspansiyonTypeService,
  ) { }
  ...
  async loadPage(page: number, forceReload = false) {
    ...
    try {
      const res = await this.suspansiyonTypeService.getAll();
  ...
  add() {
    const href = `/features/bom/suspansiyon-bom-create`;
    this.tabService.openTab({
      id: href,
      title: `Kabin Ürün Ağacı Oluştur`,
      href,
      closable: true,
      dirty: false
    });
  ...
  update() {
    ...
    const href = `/features/bom/suspansiyon-bom-detay/${row.id}`;

    this.tabService.openTab({
      id: href,
      title: `Süspansiyon Ürün Ağacı #${row.seriNo ?? row.id}`,
      href,
      closable: true,
      dirty: false,
      context: {
  ...
  async deleteSelected() {
    ...
    try {
      await this.suspansiyonTypeService.delete(this.selectedItem.id);
