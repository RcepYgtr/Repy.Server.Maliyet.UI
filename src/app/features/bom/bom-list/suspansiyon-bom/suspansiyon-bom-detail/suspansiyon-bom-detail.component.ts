...
@Component({
  selector: 'app-suspansiyon-bom-detail',
  standalone: false,
  templateUrl: './suspansiyon-bom-detail.component.html',
  styleUrl: './suspansiyon-bom-detail.component.scss',
})
export class SuspansiyonBomDetailComponent implements OnInit {
  form!: FormGroup;
  ...
  private loadFromRoute() {
    ...
    const suspansiyonTypeId = +id;

    setTimeout(() => {
      this.form.patchValue({ suspansiyonTypeId: suspansiyonTypeId });
    });

    this.loadBom(suspansiyonTypeId);
  }

  async loadBom(id: number) {

    const res: any = await this.suspansiyonBomService.getByModel(id);
  ...
  async Kaydet() {
    ...
    try {
      await this.suspansiyonBomService.update(payload);
