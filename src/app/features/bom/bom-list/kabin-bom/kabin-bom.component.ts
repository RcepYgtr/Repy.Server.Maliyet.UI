...
@Component({
  selector: 'app-kabin-bom',
  standalone: false,
  templateUrl: './kabin-bom.component.html',
  styleUrl: './kabin-bom.component.scss',
})
export class KabinBomComponent implements OnInit {
  isLoading: any;
  ...
  constructor(
    ...
    private kabinTypeService: KabinTypeService,
  ) { }
  ...
  async loadPage(page: number, forceReload = false) {
    ...
    try {
      const res = await this.kabinTypeService.getAll();
  ...
  add() {
    const href = `/features/bom/kabin-bom-create`;
  ...
  update() {
    ...
    const href = `/features/bom/kabin-bom-detay/${row.id}`;

    this.tabService.openTab({
      id: href,
      title: `Kabin Ürün Ağacı #${row.seriNo ?? row.id}`,
      href,
      closable: true,
      dirty: false,
      context: {
  ...
  async deleteSelected() {
    ...
    try {
      await this.kabinTypeService.delete(this.selectedItem.id);
