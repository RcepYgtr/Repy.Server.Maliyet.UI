...
import { MakineSasesiBomService } from '../../../../../../libs/core/api/makine-sasesi/makine-sasesi-bom.service';

type TabType = 'materials' | 'rules';
@Component({
  selector: 'app-makine-sasesi-bom-detail',
  standalone: false,
  templateUrl: './makine-sasesi-bom-detail.component.html',
  styleUrl: './makine-sasesi-bom-detail.component.scss',
})
export class MakineSasesiBomDetailComponent implements OnInit {
  form!: FormGroup;
  ...
  constructor(
    ...
    private makineSasesiBomService: MakineSasesiBomService,
  ...
  private loadFromRoute() {
    ...
    const makineSasesiTypeId = +id;

    setTimeout(() => {
      this.form.patchValue({ makineSasesiTypeId: makineSasesiTypeId });
    });

    this.loadBom(makineSasesiTypeId);
  }

  async loadBom(id: number) {

    const res: any = await this.makineSasesiBomService.getByModel(id);
  ...
  async Kaydet() {
    ...
    if (!this.form.value.makineSasesiTypeId) {
    ...
    const payload = {
      makineSasesiTypeId: this.form.value.makineSasesiTypeId,
    ...
    try {
      await this.makineSasesiBomService.update(payload);
